...
Fetch the hub and check out `main`. Run `repo-config/configure.sh check ptr727/PhotoCleaner release` from that checkout. The command checks the shared settings, the two state-dependent settings (`has_discussions` follows visibility, `default_branch` is `main`), Dependabot security features, and both the `develop` and `main` rulesets against the hub payloads, and preserves and reports `bypass_actors` without asserting them, since who may bypass a ruleset is a human decision no payload declares.
...
From the same hub checkout, run `spec/audit.py PhotoCleaner` and read its Secrets section. It resolves the required set from the hub's `spec/secrets.json` plus this repo's registry entry (`publish[]`/`types[]`/`requiredSecrets[]`) and confirms each required name exists (name only, not values) in the stores its mechanism claims. For PhotoCleaner that is the baseline App pair, the Docker Hub pair, and `CODECOV_TOKEN`, all in both the Actions and Dependabot stores, since a workflow run triggered by a Dependabot pull request reads the Dependabot store and would otherwise silently skip the coverage upload.

## Verdict and Follow-Up

A missing required item or a divergent payload is a **defect** (not operational), and an equivalent outcome in a non-standard form is a **drift finding**. N/A items are excluded, never counted as failures. Surface findings as repository issues, and land fixes as a pull request to `develop` per [GOVERNANCE.md "Branching Model"][governance-branching-model]. To re-apply the whole baseline, run `repo-config/configure.sh apply ptr727/PhotoCleaner release` from a hub checkout.
