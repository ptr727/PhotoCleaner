...
#### The Full Post-Change Set

The clean-compile task above is necessary and not sufficient. After every code change this repo runs all three, in order, before commit, and a green build alone is not the finish line:

1. The **`.NET Format`** clean-compile task (CSharpier, build, then `dotnet format style --verify-no-changes`).
2. **`dotnet test`**: the clean-compile chain never runs the xUnit suite, so a change that compiles and formats cleanly can still be broken.
3. **`dotnet husky run`**: the same style tasks the pre-commit hook runs, so a commit is never rejected by its own hook.
...
4. **Internal visibility**: Use `InternalsVisibleTo` for test and benchmark access
   ...
   ```xml
   <ItemGroup>
     <InternalsVisibleTo Include="PhotoCleanerBenchmarks" />
     <InternalsVisibleTo Include="PhotoCleanerTests" />
   </ItemGroup>
   ```
